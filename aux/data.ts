...
module Chart {
    ...
    export class LinePlot extends Base {
        public element: ID3Selection;
        constructor (element: ID3Selection) {
            super(element);
            this.element = element;
        ...
        public render(xdata: number[], ydata: number[]) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var linechart = new Chart.LinePlot(d3.select('#linePlot'));
    var x = [],
        y = [];
    for (var i=0; i<100; i++) {
        x.push(0.1*Math.PI*i);
        y.push(Math.sin(0.1*Math.PI*i));
        
    }
    #console.log(x,y);
    linechart.render(x,y);
